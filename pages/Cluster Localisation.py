...
# sort the cluster by the number of accidents
st.header("Cluster Information")
st.caption("""
The following is the information of each cluster of the road accident collected in Victoria.
""")    
cluster_info_df_sum =  clusterd_crashsite_df.groupby("cluster").agg(
    {
        "TOTAL_PERSONS": "sum",
        "INJ_OR_FATAL": "sum",
        "FATALITY": "sum",
...
display_df = cluster_info_df_sum.sort_values(by="ACCIDENT_NO", ascending=False).sort_index(axis=1)
display_df = display_df.query("LATITUDE != -1")
st.dataframe(display_df)


# histrogram of the number of accidents
st.header("Number of accidents in each cluster")
st.caption("""
The following is the histogram of the number of accidents in each cluster.
""")

fig = px.histogram(display_df, x="ACCIDENT_NO", nbins=20, 
                   title="Number of accidents in each cluster", 
    labels={"ACCIDENT_NO": "Number of accidents", "count": "Number of clusters"}
